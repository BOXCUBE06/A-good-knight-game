...
@onready var right_character_panel: PanelContainer = %RightCharacterPanel
@onready var left_character_label: RichTextLabel = %LeftCharacterLabel
@onready var right_character_label: RichTextLabel = %RightCharacterLabel
...
func apply_dialogue_line() -> void:
	...
	left_character_panel.hide()
	right_character_panel.hide()
	
	if not dialogue_line.character.is_empty():
		var translated_name = tr(dialogue_line.character, "dialogue")
		if dialogue_line.character == "Knight":
			left_character_panel.show()
			left_character_label.text = translated_name
		else:
			right_character_panel.show()
			right_character_label.text = translated_name

	# Dual Portrait Logic
	left_portrait.hide()
	right_portrait.hide()
	
	if PORTRAIT_PATHS.has(dialogue_line.character):
		var texture = load(PORTRAIT_PATHS[dialogue_line.character])
		if dialogue_line.character == "Knight":
